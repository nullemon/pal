...
const OBFUSCATED_RE =
  /\b[a-z0-9-]+(?:\s*(?:\[\s*dot\s*\]|\(\s*dot\s*\)|\{\s*dot\s*\}|\s+dot\s+|\[\s*\.\s*\])\s*[a-z0-9-]+)+\b/gi
...
export const domainOf = (text: string): string | null => {
  ...
  s = s
    .replace(/\s*(?:\[\s*dot\s*\]|\(\s*dot\s*\)|\{\s*dot\s*\}|\[\s*\.\s*\])\s*/g, '.')
    .replace(/\s+dot\s+/g, '.')
...
export const detectLinks = (text: string): DetectedLink[] => {
  ...
  for (const m of text.matchAll(URL_RE)) push(/^hxxp/i.test(m[0]) ? 'obfuscated' : 'url', m)
