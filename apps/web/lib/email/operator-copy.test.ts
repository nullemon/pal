...
describe('with nothing configured', () => {
  it('sends exactly the shipped subjects and intros', () => {
    const mail = verifyEmailMail('reader@example.com', 'tok', copyFn(DEFAULT_COPY))
...
describe('an operator who pastes markup', () => {
  it('cannot get it into the HTML body', () => {
    ...
    const mail = resetPasswordMail('reader@example.com', 'tok', copy)
  ...
  it('cannot get it into the subject line either', () => {
    const copy = copyWith({ 'email.passwordChanged.subject': `Changed ${XSS}` })
    const mail = passwordChangedMail('reader@example.com', copy)
...
describe('an operator who pastes a newline into a subject', () => {
  it('never produces a header break, because the override is normalised on the way in', () => {
    const copy = copyWith({ 'email.verify.subject': INJECT })
    const mail = verifyEmailMail('reader@example.com', 'tok', copy)
...
describe('an override that the resolver rejects', () => {
  it('falls back, so the mail still says something sensible', () => {
    const copy = copyWith({
      'email.deletion.subject': '',
      'email.deletion.intro': 'x'.repeat(5_000),
    })
    const mail = deletionScheduledMail('reader@example.com', new Date('2026-10-01T00:00:00Z'), copy)
