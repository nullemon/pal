...
import { buttonClasses, SeriesCard, type SeriesType } from '@palscans/ui'
...
/**
 * The 404 body: the code, the line, a search box and the six most-read series.
 *
 * Shared because there are two 404 files and a reader must not be able to tell which one
 * they hit. `app/not-found.tsx` renders it inside its own header and footer — Next renders
 * that file *outside* the site layout — while `app/(site)/not-found.tsx` renders it bare and
 * inherits the layout's. Duplicating the body instead is how the site-scoped one silently
 * lost the search box and the popular rail.
 */
const COVER_W = 400
const COVER_H = 600
